...
page = open('nyt_mobile.html').read()

def run_lxml_xpath():
    all_news = []
    tree = html.fromstring(page)
    articles = tree.xpath("//div[@id='articles']")
    for story in articles[0].xpath('ol/li'):
...
def run_lxml_css():
    ...
    for story in articles.cssselect('ol.li'):
        ...
        all_news.append({
            ...
            'link': (lambda x: x[0].get('href') if x is not None else '')(link),
